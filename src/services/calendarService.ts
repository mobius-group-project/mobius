/**
 * CRUD service for calendar events with client-side recurrence expansion.
 *
 * `getEvents` returns both DB rows and their virtual recurring copies in one array.
 * Recurring copies are generated by `expandRecurringEvent` (not stored in the DB) —
 * their IDs are compound integers (`${base_id}${YYYYMMDD}`) and `originalEventId`
 * points back to the DB row. Edit/delete operations must resolve the original ID first.
 * Expansion stops at `recurrenceEndDate`, `recurrenceCount` (default 50), or 3 months
 * from the start date — whichever comes first.
 */
